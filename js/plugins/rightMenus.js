...
RightMenus.fun = (() => {
  ...
  fn.menuNotic = () => {
    ...
    if (RightMenus.messageRightMenu && !NoticeRightMenu)
      VolantisApp.message('右键菜单', '唤醒原系统菜单请使用：<kbd>Ctrl</kbd> + <kbd>右键</kbd>', {
        icon: rightMenuConfig.options.iconPrefix + ' fa-circle-exclamation red',
        displayMode: 1,
        time: 9000
      }, () => {
  ...
  fn.printHtml = () => {
    ...
    DOMController.removeList([
      ...
      'footer', '#l_header', '#l_cover', '#l_side', '#r_side', '#comments', '#s-top', '#BKG',
  ...
  fn.readMode = () => {
    ...
    DOMController.fadeToggleList([
      ...
      document.querySelector('.prev-next'), document.querySelector('#l_side'), document.querySelector('#r_side'),
