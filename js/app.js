...
const VolantisApp = (() => {
  ...
  fn.event = () => {
    ...
    // 如果 rsidebar 为空，隐藏 sidebar。
    const rsidebar = document.querySelector("#r_side")
    if (rsidebar) {
      const sectionList = rsidebar.querySelectorAll("section")
      if (!sectionList.length) {
        document.querySelector("#l_main").classList.add("no_sidebar")
      }
    }
    ...
    // 站点信息 最后活动日期
    if (volantis.GLOBAL_CONFIG.sidebar.for_page_left.includes('webinfo') || volantis.GLOBAL_CONFIG.sidebar.for_page_right.includes('webinfo') || volantis.GLOBAL_CONFIG.sidebar.for_post_left.includes('webinfo') || volantis.GLOBAL_CONFIG.sidebar.for_post_right.includes('webinfo')) {
    ...
    if (volantis.dom.topBtn) {
      if (scrollTop > volantis.dom.bodyAnchor.offsetTop) {
        ...
        if (volantis.scroll.del > 0) {
          ...
          volantis.dom.topBtn.removeClass('show');
        } else {
          volantis.dom.topBtn.addClass('show');
  ...
  fn.setHeader = () => {
    ...
    if (volantis.isMobile) {
      ...
      if (!volantis.dom.tocTarget) {
        volantis.dom.tocTarget = volantis.dom.$(document.querySelector('#r_side .toc-wrapper')); // 侧边栏的目录列表
      }
      if (volantis.dom.tocTarget) {
  ...
  fn.setHeaderMenuSelection = () => {
    // !!! 此处的Dom对象需要重载 !!!
    volantis.dom.headerMenu = volantis.dom.$(document.querySelectorAll('#l_header .navigation,#l_cover .navigation,#l_side .navigation,#r_side .navigation')); // 导航列表
